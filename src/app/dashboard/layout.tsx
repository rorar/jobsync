...
import { SuperLikeCelebrationsProvider } from "@/context/SuperLikeCelebrationsContext";
...
export default function RootLayout({
  ...
  return (
    <ActivityProvider>
      {/*
        M-A-07 (Sprint 3 Stream B): `useSuperLikeCelebrations` was
        previously mounted inside `DeckView`, so `router.push` from the
        "Open job" CTA destroyed the queue by unmounting the deck. Lifting
        the queue to this layout (which is stable across all
        `/dashboard/*` page transitions, including `/dashboard/myjobs/:id`
        that "Open job" navigates to) guarantees queued celebrations
        survive navigation. See
        `src/context/SuperLikeCelebrationsContext.tsx` for the
        aggregate-boundary rationale.
      */}
      <SuperLikeCelebrationsProvider>
