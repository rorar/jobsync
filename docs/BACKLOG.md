...
|------|------------------------|---------|
| **Company-targeted task/note on the Company timeline** | ✅ **Event plumbing PRE-STAGED** (2026-06-12): the 3 payloads `CrmTaskCreated`/`CrmTaskCompleted`/`CrmNoteCreated` (+ Zod schemas + `event-bus.allium`) now carry optional `targetCompanyId`; the emitters set it for a direct company target; the projection prefers the explicit value and falls back to job→company resolution. **Read surface still gated on ROADMAP 2.21 CompanyDetail** — no company-timeline UI yet (the `CompanyTimeline` surface is already declared in `specs/crm.allium`). When 2.21 lands, just render it. | `crm-activity-logger.ts`, `event-types.ts`, `event-schemas.ts`, `event-bus.allium`, `crmTask/crmNote.actions.ts` |
| **User-Guide CRM section** | ✅ DONE — `docs/user-guide/crm.md` covers contacts, point-of-contact, recruiter triangle and the CRM timeline; extended 2026-08-06 with **"Where a contact works"** (company picker, inline create, the deliberate empty-company case, and why linking is what makes warm paths work). Row was stale: the guide already existed when this said "undocumented". | `docs/user-guide/crm.md` |
...
| **`CompanyAssociation.companyLabel` staleness** | Denormalised display cache on Person (the Allium value carries only the reference). Renaming a Company does **not** update labels already stored on Person records. Deliberate: avoids a company lookup per rendered person. Trigger for revisiting: the first user report of a stale employer name, or the BaseCombobox consolidation (§G) introducing server-side company search, which would make on-read resolution cheap. | `person.model.ts` `CompanyAssociation`, `specs/crm.allium:84` |
| **Unlinked legacy company associations** | Rows captured before the CompanyPicker (`companyId: ""` + free-text label) are preserved on edit and nudged, never auto-dropped. They stay invisible to `findWarmPaths` (exact-id match) until linked. Spec-nonconforming (`company: Company` is required) and knowingly so — to be recorded as an `open question` via `allium:tend` **together with** the unresolved-company state of CRM slice 2, not separately. | `PersonForm.tsx`, `specs/crm.allium:84`, `:1416` |
