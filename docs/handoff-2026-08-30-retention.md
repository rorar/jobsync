...
| # | Task | Where | Notes |
|---|---|---|---|
| **1** | **ADR for the retention change** | `docs/adr/` next free number | CLAUDE.md requires one after architecture changes. Covers erase-vs-archive semantics + the session-free helper extraction. Two agents flagged it as owed. |
