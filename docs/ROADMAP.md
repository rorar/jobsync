...
**Allium-Spec (DRAFT 2026-06-15):** `specs/communication-connector.allium` (via `allium:elicit`, `allium check` grün, Semantik verifiziert) — neuer ConnectorType `communication` (ACL-Pattern, Module: Email/PBX/Portal 1.9/LinkedIn), **bidirektional**: Outbound (`DeliverOutreach` + `ResolveRecipient` + `OutreachChannel` — jetzt HIER owned) und Inbound (`SyncInbox` → CRM-Timeline-Events). `MessageThread` (Konversation pro Job) + `CommunicationMessage`. **Erfüllt zwei deferred Contracts:** crm.allium `CommunicationConnector.sync` (Inbox→Timeline) + automation-modes Outbound-Delivery. **Invarianten:** `TenantIsolation`, `OutboundIsThirdPartyTransfer`, `InboundFeedsCrmNotNotifications` (G1-Boundary), `DeliveryRequiresResolvedRecipient` (G2-Dead-End → automation-modes `HoldForMissingRecipient`), `EveryMessageBelongsToAThread`. **Gefundene Gaps/Open Questions:** G1 SMTP-Transport-Sharing mit 0.6 (Boundary klar, Transport offen), G2 Recipient-Resolution-Chain (JobContact→Company), G3 Orphan-Inbound-Threading, Channel-Selection (Priority vs. Auto-Detect), Delivery-Confirmation-Tiefe (Module-Capability), **Naming-Reconcile** `Outreach.deliver`↔`DeliverOutreach` (Abstraktions-Ebenen-Mismatch, Follow-up-tend). **Cross-Refs (Specs):** crm.allium (`deferred CommunicationConnector.sync`, `email_sent/email_received`-Activities), automation-modes (`OutreachChannel`/Delivery), application-documents (`OutreachMessage`), module-lifecycle (`ConnectorType`), 0.6 notification-dispatch (Boundary).

**Autopilot-Rolle:** Eigentümer der **Outreach-Delivery** für 9.4 — Channel-Auswahl, Recipient-Auflösung, Versand; `application-documents.allium` liefert die `OutreachMessage` (Content), `automation-modes` gated nur.
...
referenziert (Spotlight 2.20, Enrichment-Panel E1.1), aber **noch keine eigene Seite**.
...
- **Company-Timeline (Welle-3-Anschluss):** Die `ActivityTimeline` akzeptiert bereits `targetCompanyId`
  ...
  *company-getargetete* Tasks/Notes (Target = Company direkt, nicht über einen Job) erscheinen noch
  ...
  `targetCompanyId` tragen. **TODO bei 2.21:** die 3 Payloads (+Schemas) additiv um optionales
