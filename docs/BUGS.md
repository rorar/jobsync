...
> | Session 2026-09-05 (verification pass, `SPEC-B3`) | 1 | 0 | 1 | — |
> | **Total** | **656** | **640** | **14** | **2** |
>
...
| SPEC-B3 | LOW | **`allium check` and `allium analyse` are both blind to reference integrity — but the language server is not.** `specs/module-lifecycle.allium` declares surface `NotificationInbox` with `provides: MarkAsRead / MarkAllAsRead / DismissNotification`, and no rule anywhere listens for those triggers (`grep -c "when: MarkAsRead"` = 0). `allium check specs/` reports **0 errors** and `allium analyse` names none of them; the Allium LSP reports all three as `allium.surface.undefinedProvidesTrigger`. Found 2026-09-05 while tending the spec — the diagnostics fired on a diff that added only comment lines, which is how the pre-existing defect surfaced. | **Open.** Two separable things: the three dangling triggers (a real spec defect — either the rules are missing or the surface overpromises), and the tooling lesson. CLAUDE.md § Specification Pattern says a green `allium check` proves syntax only and that formal clauses "are unresolved too"; that stands for the CLI and is now known to be FALSE of the LSP. A reference-integrity oracle exists and nothing in this project's gates runs it. |
