...
| E2E-B35 | LOW | **`keyboard-ux.spec.ts` is a timing-sensitive cluster, not a set of independent flakes.** Across four full runs its tag/combobox tests failed in varying combinations (`:260`, `:315`, `:353`, `:454`, `:679`) and **every one of them passed in isolation**, repeatedly, in a fraction of the time (2.5 s isolated vs 13.8 s to timeout in a full run). The file is the largest in the suite (22 tests, 903 lines), runs mid-suite, and asserts on `sr-only` announcements with 5 s polling budgets — the first thing a loaded machine breaks. Recorded as one finding so the next run does not re-diagnose each member separately. The real lever is not the timeouts: the dev server reaches 4-5 GB RSS during a run, and full-run duration moved 15.1 → 19.2 min between two runs with identical code. | Open |
