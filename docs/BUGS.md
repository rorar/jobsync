...
| E2E-B39 | MEDIUM | **A test that only passed because an earlier test in the same file left data behind.** `profile-crud.spec.ts:498` ("edit experience dialog opens and cancels") reaches the Job Title combobox expecting to SELECT `"Software Developer"`, which an earlier test in the file creates and never deletes. Adding cleanup made it take the CREATE path instead, and the create path leaves the trigger empty: `toContainText` reads `""`. Measured both with the shared fixed name and with a uid-suffixed one, so it is the path and not the name. Two defects meet here — a cross-test dependency (`NoCrossTestDependency` in the spec) and a create path that does not populate the control — and neither is a cleanup hook's to fix, so `profile-crud`'s net deliberately does NOT sweep `JobTitle` and says so at the call site. | Open |
| E2E-B40 | MEDIUM | **A cleanup helper that proves a deletion with an assertion a filter can satisfy.** `task-crud.spec.ts` de-registers a task once `expect(taskRows(page, title)).toHaveCount(0)` holds — but the task list is status-filtered and `DEFAULT_STATUS_FILTER` resets on every `goto`, so a task that merely left the current filter also has count 0. The `afterEach` net then believes the row is gone and never warns: measured after a green 16/16 run, six tasks remain in the database (`in-progress`, `needs-attention`, `complete`) with no warning in the log. Same class as E2E-B20, where a toast assertion was satisfiable by the previous action's toast. A count over a filtered view is not evidence of deletion; the check has to be against something the filter cannot change. | Open |
