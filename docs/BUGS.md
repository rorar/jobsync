...
**Total: 650 bugs found, 632 fixed, 19 open (2 accepted risk + 1 from 2026-09-01 + 16 from 2026-09-02/03)**

> The three figures above do not reconcile (632 + 19 = 651, not 650) and have not reconciled
> for several sessions. The off-by-one is inherited, not introduced here; the 2026-09-01 edits
> added 21 findings and closed 18 of them (only E2E-B11 still open: B9 and B12 were closed by the 2026-09-02 disposable run database), and the 2026-09-02
> ownership audit added 18 (E2E-B22..B37 plus SPEC-B1/B2), of which 5 are fixed — both preserve the discrepancy
...
## Session 2026-09-01/02 — E2E fixture extraction, the data leak behind it, and the close-out (21 found, 18 fixed, 3 open)
...
Only E2E-B9, B11 and B12 below are open. The four rows marked **FIXED** are kept in this table
rather than moved, because each one's cell records what the original finding got WRONG about
itself, and that correction is the useful part — but count from the bold FIXED markers, not from
the row count.
...
## Session 2026-09-02 — E2E ownership audit (18 found, 5 fixed)
...
| E2E-B37 | LOW | **"Ends at zero" was a property of the run being green, not of the spec owning its rows — measured, not argued.** The Phase 2 measurement showed `SmtpConfig`, `CompanyBlacklist` and `Question` at zero residue and that was read as evidence those specs clean up reliably. A reviewer pointed out that all three clean up **inline at the end of the test body**, the path a failed assertion skips, and that only the webhook and API-key specs have `afterEach` nets. Falsification test run 2026-09-02 21:40: one `expect(true).toBe(false)` injected between save and the inline delete in `smtp-settings.spec.ts:202`, run through the wrapper — `SmtpConfig` goes from 0 to **1**, an active per-user singleton left in the run database. The suite absorbs this particular case because `ensureEmptyState` (`:72-78`) deletes an existing config before each SMTP test; a model without such a guard would not be absorbed. The inference "the capacity-bound models already own their rows, so a fixture rewrite buys nothing" therefore does not follow from the measurement. What survives: those rows still die with the run database, and a count-based residue gate DOES catch this shape (an INSERT). It does not catch the UPDATE shape — see the `deactivateModule` upsert in the same session's notes. | Open |
