...
**Total: 658 bugs found, 646 fixed, 10 open, 1 closed by its premise disappearing, 1 reclassified as a corrected assumption**
...
> | Session 2026-09-02 (`E2E-B22..B41`, `SPEC-B1/B2`) | 22 | 14 | 6 | 1 closed (`E2E-B26`), 1 corrected assumption (`E2E-B37`) |
...
> | **Total** | **658** | **646** | **10** | **2** |
>
> 646 + 10 + 2 = 658. **Moved 2026-09-07:** `E2E-B35` open → fixed (its block row 12/8 → 13/7). **Corrected 2026-09-06:** this line and the header above still carried the
...
| E2E-B24 | MEDIUM | **Widened 2026-09-02 22:30 — the residue gate found three the audit missed:** `Task` (+6 per run), `Activity` (+1), `ActivityType` (+1). Those DID have backstop steps (2, 3, 12) in the since-deleted purge, so they leaked inside every run and were swept between runs — the purge was hiding them, not preventing them. `task-crud` and `activity-crud` delete inline, the path a thrown assertion skips; `ActivityType` was never deleted inline at all. Tracked as KNOWN_DEBT in `scripts/check-e2e-residue.sh`, which prints them every run. Original finding — models written by specs with **no backstop step whatsoever**: `Tag` (`keyboard-ux.spec.ts:320,364,406,876`, `question-crud.spec.ts:112`), and the resume children `Summary`, `WorkExperience`, `Education`, `LicenseOrCertification`, `OtherSection`. Step 7 deletes `ResumeSection` but those children hang off optional FKs (`schema.prisma:165-265`), so deleting the section orphans rather than removes them. **`Task` and `Activity` fixed 2026-09-04** (`278ad940`, `8c72d743`): neither was the thrown-assertion shape described here, both were E2E-B40's aria-hidden shape — the delete helper could not see the row it was asserting gone. Measured `Task +6 -> 0`, `Activity 0`. `ActivityType` remains and has no delete path anywhere in the application, so it can only be reduced by creating fewer distinct rows, which both specs now do. **Read `278ad940`'s message carefully — it has already been misread once.** It says "The residue gate no longer lists Task". That is true of the gate's **OUTPUT**: `Task` residue reached zero, so the gate printed nothing for it. It is **not** true of the gate's **INPUT**: `"Task:E2E-B24"` is still in the `KNOWN_DEBT` array at `scripts/check-e2e-residue.sh:95`, verified 2026-09-04, alongside `"Activity:E2E-B24"` at `:96`. The difference is load-bearing and the array's own rule states it at `:83` — "the entry is deleted when the finding is". While the entry remains, a **regression** in `Task` prints as known debt with a finding number instead of failing the run: the gate would classify it rather than reject it. Deleting the two entries is what actually converts the measured fix into enforcement, and it has not been done. | Open |
| E2E-B25 | MEDIUM | `keyboard-ux.spec.ts` creates reference data with names that carry **no `E2E ` prefix and no cleanup**: `KBTest Title/Co/Loc` (`:143,179,200`), `KBRapid` (`:264`), `KBMobile` (`:786`), plus tags. Only its four resumes are cleaned (`:512,557,596,634`). `profile-crud.spec.ts` does the same with `"Software Developer"`, `"company test"`, `"location test"`, `"Boston"`, `"Cambridge"` (`:203-224,265-289,374-390,511`). **profile-crud's half fixed 2026-09-04** (`8c72d743`): its teardown was blind to the rows it was deleting (E2E-B40), and with that repaired a profile-crud run ends at zero for `Resume`, `ResumeSection`, `ContactInfo`, `Summary` and `Location` — measured `+3/+1/+1/+1/+3 -> 0` on an otherwise identical run. `JobTitle +2` is NOT a failure of that net: the group is commented out at `profile-crud.spec.ts:269` on purpose, because sweeping it turns E2E-B39 red. keyboard-ux's unprefixed names are untouched and were not re-measured. | Open |
