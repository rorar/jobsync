...
| E2E-B18 | MEDIUM | Step 0b has a resurrection race: `getModuleManifests` health-checks any module whose in-memory health is `UNKNOWN` with no status filter (`module.actions.ts:79-86`), and `checkModuleHealth`'s create branch writes `status: registered.status` (`health-monitor.ts:215-227`). A process holding a stale `inactive` can therefore re-persist it into the table step 0b just emptied. Live for `google_favicon` (declares a `healthCheck`), not for `jsearch` (early-returns). | Largely defused by the fresh-server-per-run change, since a new process holds no stale status — but not eliminated. Documented in ADR-043 § Consequences/Negative (`71d45e51`); the code paths are unchanged, so the finding stays OPEN. |
