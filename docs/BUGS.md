...
**Total: 658 bugs found, 642 fixed, 14 open, 1 closed by its premise disappearing, 1 reclassified as a corrected assumption**
...
> | Session 2026-09-06 (production build, `E2E-B43`) | 1 | 0 | 1 (`E2E-B43`) | — |
> | **Total** | **658** | **642** | **14** | **2** |
>
> 642 + 14 + 2 = 658. **Corrected 2026-09-06:** this line and the header above still carried the
...
| E2E-B43 | MEDIUM | **A test that passes against the dev server fails against a production build — the first finding the production move surfaced.** `keyboard-ux.spec.ts:755` "Multiple tags can be added rapidly via Enter" types three skills into the AddJob dialog's `TagInput`, pressing Enter after each and waiting for that skill's chip before typing the next. Under `E2E_PROD=1` it fails on the SECOND skill: `getByText('KBMulti2 <uid>')` never becomes visible within 10 s. Reproduced twice — as test 54 of the full 112-result run and running the spec ALONE (29 passed / 1 failed, 1.6 min) — so it is not contention, and not the ordering sensitivity E2E-B35 describes. The same spec against the DEV server passes that test (test 17 ✓ in an isolated dev run the same evening), which is what makes this a dev/prod difference rather than a flake. **What the failure state shows.** Playwright's error context has the Skills tab panel holding only its heading and the "Add Skill" button: the popover is closed and there are NO chips at all — including skill 1's, whose visibility assertion had already PASSED seconds earlier. Chips render from `selectedTags = localTags.filter(...)` (`TagInput.tsx:45`) where `localTags` is seeded once from the prop (`:40`), so "no chips" plus "popover closed" (`open` back to its `useState(false)` initial, `:38`) is the shape of a REMOUNT of `TagInput`. **That is a shape, not a cause.** Nothing here identifies what remounts it. The obvious candidate is already ruled out: `createTag` (`src/actions/tag.actions.ts:61-86`) calls no `revalidatePath`, so no router refresh follows the create. Untested and worth testing next: whether `handleCreate`'s `startTransition` around an async server action (`TagInput.tsx:73`) schedules differently under a production React build, and whether the AddJob dialog's tab content unmounts. No cause is recorded in this row on purpose — E2E-B42 took three wrong diagnoses written in the register of findings, and this row is not going to be the fourth. | Open |
