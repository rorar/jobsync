...
> | Session 2026-09-02 (`E2E-B22..B41`, `SPEC-B1/B2`) | 22 | 8 | 12 | 1 closed (`E2E-B26`), 1 corrected assumption (`E2E-B37`) |
> | `MOD-B1` — pre-existing, recorded in the first table but belonging to neither audit | 1 | 0 | 1 | — |
> | **Total** | **655** | **637** | **16** | **2** |
>
...
### Open — 1 item (plus MOD-B1, which belongs to no audit)
...
|---|---|---|---|
| MOD-B1 | MEDIUM | **Silent success over a lost write in module lifecycle.** `handleAuthFailure` sets the in-memory status to ERROR (`degradation.ts:89`), then wraps its own DB write in a try/catch that logs and **continues** (`:101-103`) — it goes on to pause automations and returns a success-shaped result while the module's error state lives only in memory. `deactivateModule` has the same memory-before-DB order (`module.actions.ts:345` then `:348`) with no rollback on throw. Then `module.actions.ts:337-342` early-returns `success: true` when memory already says INACTIVE, so re-issuing the deactivation writes nothing and reports success. Recovery requires activate-then-deactivate, which nobody would guess. | Open, pre-existing, found 2026-09-02 while auditing `f56da9ff`. Not introduced by that commit. It was recorded only as a Negative bullet inside ADR-044, which is where findings go to be forgotten — hence this row. Fix shape: persist before mutating memory, or roll memory back on write failure; and make the early return distinguish "already inactive in DB" from "inactive in memory only". |
