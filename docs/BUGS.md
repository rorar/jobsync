# Bug Tracker — Collected 2026-03-24, Updated 2026-09-07

**Total: 658 bugs found, 647 fixed, 9 open, 1 closed by its premise disappearing, 1 reclassified as a corrected assumption**

> **Recounted 2026-09-04**, as the previous version of this line asked. It reconciles now, and here
> is the arithmetic so the next reader can check it rather than inherit it.
>
> **What was actually recounted: the two E2E sections only** (`Session 2026-09-01/02` and
> `Session 2026-09-02`). The 611 findings recorded before them were **NOT** recounted — that is
> 1400 lines in half a dozen incompatible formats, and a claim about them here would be an
> assertion, not a count. They are carried forward on the file's own word, contributing 2 open
> (the "accepted risk" pair) and 609 fixed.
>
> | Block | Found | Fixed | Open | Other |
> |---|---|---|---|---|
> | Pre-E2E backlog — **carried forward, not recounted** | 611 | 609 | 2 | — |
> | Session 2026-09-01/02 (`E2E-B1..B21`) | 21 | 20 | 1 (`E2E-B11`) | — |
> | Session 2026-09-02 (`E2E-B22..B41`, `SPEC-B1/B2`) | 22 | 15 | 5 | 1 closed (`E2E-B26`), 1 corrected assumption (`E2E-B37`) |
> | `MOD-B1` — pre-existing, recorded in the first table but belonging to neither audit | 1 | 1 | 0 | — |
> | Session 2026-09-05 (verification pass, `SPEC-B3`) | 1 | 1 | 0 | — |
> | Session 2026-09-06 (heap attribution, `E2E-B42`) | 1 | 0 | 1 (`E2E-B42`) | — |
> | Session 2026-09-06 (production build, `E2E-B43`) | 1 | 1 | 0 | — |
> | **Total** | **658** | **647** | **9** | **2** |
>
> 647 + 9 + 2 = 658. **Moved 2026-09-07:** `E2E-B35` open → fixed (its block row 12/8 → 13/7). **Corrected 2026-09-06:** this line and the header above still carried the
> recount's FIRST numbers (655 / 637 / 16) after later rows moved the block table to 657 / 642 / 13.
> The block table was right and is the derivation; the two summaries had simply not been carried
...
> that: it was counted among the 19 open (the "16 from 2026-09-02/03" = `MOD-B1` + 15 section rows)
> while being a pre-existing finding that neither audit "found", so it plausibly never entered the
> 650. Not proven — proving it needs the 611 recount — but it is the only unmatched item this pass
> turned up, and it is now counted explicitly on both sides.
>
> Since the previous line was written, `E2E-B38..B41` were added (2026-09-03/04) and four entries
> changed disposition: `E2E-B29` and `E2E-B30` to Fixed, `E2E-B26` to Closed, `E2E-B37` to a
...
the suite. The refactor itself was never implicated — no failure in any of the three runs had a
code path to it.

| ID | Severity | Bug | Fix |
|---|---|---|---|
| E2E-B1 | HIGH | Leaked `WebhookEndpoint` rows accumulate to the cap of 10 (`webhook.actions.ts:19,110`), at which point `WebhookSettings.tsx:275,344` renders the create form **disabled** — so every webhook test fails in every later run, permanently, presenting as an app bug rather than stale data. Reached exactly 10 on 2026-09-01. | `test.afterEach` safety net in `webhook-settings.spec.ts`, registering inside `createWebhookEndpoint` so no caller can forget, de-registering on a successful inline delete. |
| E2E-B2 | HIGH | `cleanup-stale-data.ts` never purged `WebhookEndpoint` at all — the leak had no backstop. | New step 0a. Placed at the **front**: any later step that throws aborts the function, so the steps preventing a permanent outage must not be reachable-only-on-success. |
| E2E-B3 | MEDIUM | `profile-crud.spec.ts` names its resumes `Resume Create ${uid}` etc. with no `E2E ` prefix, while the purge filters `title: { startsWith: "E2E " }` — those rows were cleaned by nothing, ever. Recorded 2026-08-31 as finding B and left open. | All nine title literals prefixed. |
| E2E-B4 | HIGH | `ModuleRegistration` is global (no user column), carries no `E2E ` marker and was in no cleanup. A leaked `inactive` row makes `automation-wizard-modules.spec.ts` and `enrichment.spec.ts` skip their own subject and pass **vacuously** — green while testing nothing. Both were in that state on 2026-09-01 (`jsearch`, `google_favicon`). | New step 0b: `deleteMany({})`, unfiltered. Deliberately deletes rather than writing `active`: `module.actions.ts:439-450` only iterates rows that exist, so absence restores the manifest default from `registry.ts:67`. No status policy, no module list, new modules covered automatically. |
| E2E-B5 | LOW | `cleanup-stale-data.ts` bypassed `prisma.$disconnect()` on the "no test user" early return and on any throwing `deleteMany`. | Body extracted to `deleteStaleRecords()`, wrapped in `try/finally`. Extraction over indentation so the diff shows behaviour, not whitespace. |
...
| E2E-B14 | HIGH | The `expect(wasActive).toBe(true)` precondition this branch added would have broken CI. `playwright.config.ts:14` is `retries: process.env.CI ? 2 : 0`; attempt 1 deactivates JSearch, and attempts 2-3 then fail the precondition because `syncRegistryFromDb` reads ModuleRegistration once per process and the module is credential-gated, so nothing restores it. One honest failure becomes three with the real one buried. Only ever visible in CI. | `test.describe.configure({ retries: 0 })` on that block — a retry cannot succeed here by construction, so it only destroys evidence. |
| E2E-B15 | MEDIUM | Both `afterEach` safety nets were unable to report. `deleteWebhookEndpoint` and `purgeApiKey` wrap their bodies in a bare `catch {}`, so they never throw, so each hook's own `catch (error) → console.warn` was reachable only if the initial navigation failed. A row the net FAILED to delete produced no warning and no failure — the exact inverse of the comment beside it. | Both hooks now re-check with `count()` after the deletion attempt and warn on what actually survived, naming the cap it counts against. |
| E2E-B16 | MEDIUM | The five `.animate-spin` waits added by this branch could lock onto the wrong spinner: `SchedulerStatusBar.tsx:30` renders one whenever a scheduler run is active and `Header.tsx:76` mounts it above `<main>` in DOM order, so a page-wide `.first()` waits on the header pill, times out, and is swallowed. Silently degrades to the race it was added to remove. Exposure measured: one active automation, hourly cron, ~27 min suite, so roughly half of runs. | Scoped to `getByRole("main")` at seven sites — the five new ones plus the two pre-existing (`smtp-settings:52`, `push-settings:70`) that are the precedent the others copied. |
| E2E-B17 | HIGH | Dev-server reuse was silently unsound. Module activation lives in the server PROCESS behind the `dbSynced` latch, so step 0b has no effect on a server that already synced, and `automation-wizard-modules` failed its own precondition on the SECOND run against one process. Measured: same tree, reused server 1 failed, fresh server 10 passed. | `scripts/test-e2e.sh` now starts a fresh server per run, waiting for the old one to go down first so it cannot mistake the dying process for the new one. `E2E_REUSE_SERVER=1` opts out and says what it gives up. |
### Open — 1 item (`MOD-B1` closed 2026-09-05; see its row)

**Recounted 2026-09-04.** Of the nine rows below, only **`E2E-B11`** is open, plus `MOD-B1` — a
pre-existing product defect recorded here for want of a better home, not one of this session's 21
findings. `E2E-B9` and `E2E-B12` were closed by the 2026-09-02 disposable run database; `E2E-B10`
is resolved as far as the toolchain allows; `E2E-B18`, `E2E-B19`, `E2E-B20`, `E2E-B21` are fixed.
The heading previously said "3 items", which was true when written and stopped being true the same
day. The rows marked **FIXED** are kept in this table rather than moved, because each one's cell
records what the original finding got WRONG about itself, and that correction is the useful part —
but count from the bold FIXED markers, not from the row count.


| ID | Severity | Bug | Why not fixed |
|---|---|---|---|
| MOD-B1 | MEDIUM | **FIXED (`f721f693` + 2026-09-05 follow-up) — it took two halves, and the first commit shipped one of them.** The defect: memory-before-DB ordering plus a short-circuit that trusted memory alone, so a lost write was reported as success and every retry repeated the lie. `f721f693` reordered all three lifecycle-status sites (`module.actions.ts` activate + deactivate, `degradation.ts:118` handleAuthFailure) and added the DB-agreement check to `deactivateModule` only — while its message claimed `activateModule` "is fixed the same way". True of the ordering, false of the short-circuit. The follow-up adds the mirror check at `module.actions.ts:192`, where an ABSENT row AGREES (schema default is `active`), which is the opposite of the deactivate case. | **Fixed.** Reachability is not the failed write — that path self-repairs — but stale memory: `syncRegistryFromDb` latches on `dbSynced` and reads the deployment-global `ModuleRegistration` ONCE per process. Regression tests: `__tests__/module.actions.spec.ts` (absent row short-circuits, disagreeing row repairs, throwing read reports failure). Residual, NOT fixed: a throw after the persist leaves the record written with nothing paused — see the note in `degradation.ts`. |
| E2E-B9 | ~~MEDIUM~~ | **FIXED by the disposable run database.** A fresh database has no `ModuleRegistration` row at all, so JSearch resolves to its manifest default ACTIVE and the spec's precondition holds without any reset. The credential gate below is still real; it simply no longer matters to the suite. Original: JSearch cannot be re-activated through the UI on this host: `credential.type: API_KEY, required: true`, and `activateModule` refuses without a default, `RAPIDAPI_KEY`, or a stored `ApiKey` row. The per-test restore that used to hide this was removed, so the module stays inactive for the rest of the dev-server process (`dbSynced` syncs once). | Needs a decision: seed a credential fixture, set the env var for the E2E server, or accept step 0b as the reset. Step 0b already makes it self-healing across runs — **but only through the wrapper.** Since 2026-09-02 step 0b is gated on `E2E_ALLOW_DESTRUCTIVE=1`, which only `scripts/test-e2e.sh` sets, so a HAND-RUN of Playwright skips the reset. After a run that died mid-toggle, `automation-wizard-modules.spec.ts` then fails its `wasActive` precondition and cannot recover by hand, because of exactly the credential gate this row describes. Re-run through the wrapper. |
| E2E-B10 | MEDIUM | Playwright 1.57.0 expects chromium build **1200** (`playwright-core/browsers.json`); the cache holds only **1234**. Without `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` every run dies at `global-setup.ts:15` before a single test — a launch failure, not a test failure. | **RESOLVED as far as it can be (`41b87e09`).** The planned fix does not exist: `npx playwright install chromium` FAILS here with `Playwright does not support chromium on ubuntu26.04-x64`, and `--dry-run` prints an install location but no download URL — playwright-core 1.57.0 has no ubuntu26.04-x64 build for revision 1200 at all, so the pinned build is unobtainable until `@playwright/test` is upgraded. `scripts/test-e2e.sh` now falls back to the newest cached chromium and prints, every run, which build it used, which one the package pins, and that an upgrade — not a re-install — removes the skew. Hard stop retained for an empty cache; both branches exercised standalone before commit. Still true that every E2E number is measured against 1234 rather than 1200. |
| E2E-B11 | LOW | React hydration mismatch in the dashboard chrome: three Radix `useId` values diverge between server and client (Sheet trigger, NotificationBell, ProfileDropdown `aria-controls`/`id`). **Root-caused 2026-09-01 to Next.js dev-server tree shape — NOT app code.** Caught TWICE, not once (`/tmp/e2e-extract-3.log:159` = `keyboard-ux.spec.ts:454`, `/tmp/e2e-crud-run-1788275468.log:153` = `:599`), with byte-identical ids both times: deterministic in shape, intermittent in occurrence (2 of 6 runs). Decoding the React 19 tree-ids (`pushTreeId`, `react-dom-client.development.js:5103-5131`: parent bits low, `index+1` above) gives `server_upper = client_upper * 4 + 2` for all three components — exactly ONE extra 2-or-3-child array fork on the SERVER at index 1. Verified independently by re-decoding the base-32 ids from the logs. NOTE on that re-decode: a naive longest-common-bit-suffix search returns 27 bits, but that is a numerical artefact, not a structural correction — the extra server fork carries the value `0b10` and so does the genuine fork immediately above it (the outer `div` of the dashboard layout, index 1 of 2), so the two merge. The structurally correct shared path is **25 bits**, as first analysed. The distinction matters: on the 27-bit reading the extra fork would sit ABOVE the outer div, i.e. INSIDE `dashboard/layout.tsx`, and the "not app code" conclusion would collapse. Both sides emit `_R_`-form ids, so React really was hydrating. The shared path places the divergence immediately above `src/app/dashboard/layout.tsx`'s default export; the only app-owned nodes in that gap are `ActivityProvider` and `SuperLikeCelebrationsProvider`, both single-child, so no app node can produce the fork. Structure confirmed by reading: outer `[Sidebar, div]` with the layout at index 1, inner div's 5 children with `Header` at index 0, `Header.tsx`'s 6 children with the bell at 4 and profile at 5. **next-themes is exonerated** — it renders `[ThemeScript, children]`, precisely the 2-child/index-1 shape being hunted, but it sits above the divergence and its bits matched on both sides. Leading hypothesis (unconfirmed): Next emits every layout segment as `jsxs(Fragment, {children: [layerAssets, clientSegment]})` (`create-component-tree.js:651-657`, verified present) whose client twin differs in dev; `getLayerAssets` derives from the Turbopack chunk manifest, which mutates as routes compile lazily. All classic triggers were already ruled out with file:line, and the bit pattern supports that: those produce VALUE mismatches, not POSITIONAL ones. | **Open, but no code fix is warranted.** Zero occurrences in six production-build samples, and the divergent fork is provably outside app code, in a region Next gates on `NODE_ENV !== 'production'`. H1 is not PROVEN — confirming it needs a repro that captures the raw SSR HTML at the moment it fires. What remains is a decision, not a defect: `filterCriticalErrors` (`keyboard-ux.spec.ts:90-97`, used at 7 assertion sites) treats any dev-server React warning as a product-quality signal, so this framework artefact reds the suite roughly 1 run in 3. Suppressing it narrowly would also blind the suite to a FUTURE app-caused hydration bug, since the message cannot distinguish the two — deliberately left to the operator. A cold-compile explanation (H3) was considered and is DEAD: `automation-crud` runs as tests 13-17 in both failing runs and navigates to `/dashboard/automations` (`automation-crud.spec.ts:17`), so the route was warm for ~30 tests before `keyboard-ux` hit it. Warming the route would change nothing. The principled fix is already a roadmap item — **ROADMAP §8.5 Phase 3**, "Production Build (`next start`) statt Dev Server für stabilere parallele Runs" (`docs/ROADMAP.md:2639`). This entry supplies the blocking analysis for it rather than proposing something new. Note for whoever implements that phase: it also plans `retries: 1` (`:2637`), and `automation-wizard-modules.spec.ts` carries a deliberate `test.describe.configure({ retries: 0 })` because a retry there runs against a process the first attempt already mutated. Running E2E against a production build is where the oracle becomes meaningful, since the tree is deterministic there — **no longer blocked, as of 2026-09-06.** Each obstacle this row named, answered in turn: `E2E_AUTH_RATE_LIMIT_BYPASS` is still deliberately inert under `NODE_ENV=production` and still must never be set there — but it stopped being a blocker when the budget was COUNTED rather than assumed. The limit is 5 signins per 15 minutes per IP (`src/lib/auth/auth-rate-limit.ts:24-25`) and a run now spends **two**, because `e2e/global-setup.ts` mints the JWT session cookie instead of signing in (no `Session` model; `src/auth.config.ts:13` augments `@auth/core/jwt`). `playwright.config.ts` no longer hardcodes `bun run dev` — its `webServer` follows the mode. And the build's footprint is measured rather than feared: `scripts/e2e-prod-build.sh` builds through `build-safe.sh`'s 7 G cgroup in 1 min 54 s, into `.next-e2e/`, and only when a source file is newer than `BUILD_ID`. `E2E_PROD=1 ./scripts/test-e2e.sh` is the whole interface. **Observed since:** 15 production runs that executed this file's tests on 2026-09-06 produced zero hydration warnings and no failure at `:454`/`:599`. That is consistent with the row's analysis and it is NOT a proof of absence — the four dev runs the same evening were also clean, which is exactly what a ~1-in-3 rate does to a sample of four. What remains is the same decision, now cheap instead of blocked, and still untaken: either run the suite in production mode when the hydration oracle is supposed to mean something (the tree is deterministic there), or gate the assertion on build type. The second is still the honest middle step for anyone who keeps running the default dev path. |
| E2E-B12 | ~~LOW~~ | **FIXED by the disposable run database — the purge it was about no longer exists.** `e2e/cleanup-stale-data.ts` was deleted; a mode that bypasses `globalSetup` can no longer inherit residue, because there is none to inherit. What remains is a different hazard, now documented in CONVENTIONS: a mode that bypasses the WRAPPER runs against whatever `DATABASE_URL` the shell has. Original: Playwright's UI mode, watch mode and test-runner MCP call `globalSetup` once per session, so re-runs execute with no stale-data purge. Watch mode is the sharp one — three saves against `webhook-settings.spec.ts` would reach the cap. | Unreachable on this host today (no Playwright VS Code extension, `DISPLAY`/`WAYLAND_DISPLAY` unset, no `--ui`/`--watch` in the repo). Recorded as a note in `e2e/CONVENTIONS.md`, deliberately without machinery against it. |
| E2E-B18 | ~~MEDIUM~~ LOW | **FIXED, and the finding was wrong about its own mechanism.** As recorded, a process holding a stale `inactive` would re-persist it through the background `checkModuleHealth` that `getModuleManifests` fires with no status filter (`module.actions.ts:79-86`). That cannot happen: `checkModuleHealth` early-returns for any module that is not `ModuleStatus.ACTIVE` (`health-monitor.ts:110-118`), and that guard has been there since `32426cca` (2026-03-29) — so the stale process never reaches the upsert. The narrower race offered in its place — a concurrent `deactivateModule` inside the probe's await window — is **also overstated**: the mechanism is real but every lifecycle writer upserts its own status in BOTH branches, so both orderings converge on the intended value. The justification that survives is a **spec-code divergence**: `specs/module-lifecycle.allium:765-768` cites "writes only module.healthStatus … NOT module.status" as the reason `runHealthCheck` is excluded from the admin gate, and that was false for the create branch for five months (`32426cca` → `f56da9ff`). Recorded in **ADR-044**, with the one narrow ordering where the fix loses information. | Closed by removing `status` from the create payload so the schema default supplies it (`prisma/schema.prisma:606` = `registry.ts:67` = the manifest default). Health observes; lifecycle is asserted only by the admin-gated activate/deactivate. Two regression tests in `__tests__/health-monitor.spec.ts`: one asserts the create payload carries no `status`, one pins the guard so a future relaxation cannot silently re-open the path. ADR-043's paragraph on this — written the same day, by me, repeating the same false mechanism — corrected in place. |
...
## Session 2026-09-02 — E2E ownership audit (18 found by the audit; 22 rows now, 13 fixed, 7 open — see the recount at the end of this section and the block table at the top of the file)
...
|---|---|---|---|
| E2E-B22 | ~~HIGH~~ LOW | **Re-scoped 2026-09-04: this is not a test-hygiene defect, and no change to any spec can fix it. Person rows are structurally undeletable through the application.** Verified against the tree: `src/actions/person.actions.ts` exports `createPerson`, `getPerson`, `getPersons`, `updatePerson`, `archivePerson`, `reactivatePerson`, `withdrawConsent`, `reinstateConsent`, `anonymizePerson`, `mergePersons` (`:107,202,238,316,434,464,505,548,594,618`) — **there is no `deletePerson`.** The only `prisma.person.delete` anywhere in `src/` is `person.actions.ts:847`, inside `mergePersons`, removing the LOSER of a merge. Anonymisation is terminal and scrubs PII but ends in `prisma.person.update({ … status: "anonymized" })` (`src/lib/crm/anonymize-person.ts:197-201`) — **the ROW remains**, so it does not reduce a row count. The UI offers the same two verbs and no third (`PersonDetailClient.tsx:7,109,129,282,334`). The specs already knew: `contact-crud.spec.ts:23` and `:76` say "No hard delete exists for Person (GDPR design) — cleanup archives the contact". Original finding, still accurate as description: four specs name their person `` `E2E${uid}` `` with **no trailing space** (`contact-crud.spec.ts:60`, `contact-company-link.spec.ts:37`, `inside-track-crud.spec.ts:84`, `job-crud.spec.ts:453`) while the deleted backstop filtered `firstName: { startsWith: "E2E " }`. **What bounds the damage now:** the run database is disposable (ADR-045), so the rows die with the run — cross-run accumulation, which was the whole mechanism, is gone. Within a run the rows are `invisible` per `IntraRunVisibility` (`specs/e2e-test-infrastructure.allium:286-294`): `Person` carries no `@@unique` (only `@@index([userId])` / `@@index([userId, status])` in `schema.prisma`), and its one cap is `maxPersonsPerUser: 10000` (`person.model.ts:346`, enforced `person.actions.ts:117-120`) against a measured `Person +4` per run — three orders of magnitude of headroom. **Severity HIGH → LOW.** HIGH was assigned by analogy to E2E-B1, where the leak reached a cap of **10** and permanently disabled the create form; neither half of that analogy holds here (cap 10000, and no cross-run accumulation to reach it with). **Open question, deliberately not resolved here:** whether `Person:E2E-B22` stays in `KNOWN_DEBT` (`scripts/check-e2e-residue.sh:93`) with its reason corrected from "four specs archive but never delete" to "no delete path exists", or moves to `docs/NOT-PLANNED.md` as deliberately-not-fixed with a re-evaluation trigger. See the recommendation in the closing note of this section. | Open |
| E2E-B23 | ~~MEDIUM~~ LOW | **Re-scoped 2026-09-04, same shape as E2E-B22 and for the same reason: `Referral` has no delete path in the application.** `src/actions/referral.actions.ts` exports `recordInsiderTip`, `recordNetworkTip`, the five lifecycle transitions `engageReferral`/`relayReferral`/`reviewReferral`/`declineReferral`/`reviveReferral` (`:253-269`), `commitReferralToApply`, `getReferral`, `listReferrals` — **no `deleteReferral`**, and there is no `prisma.referral.delete` anywhere in `src/`. Even the GDPR cascade only severs links: `anonymize-person.ts:169-194` runs four `prisma.referral.updateMany` calls nulling `forwardedToId`/`insiderId`/`tipsterId` and keeps the row. `inside-track-crud.spec.ts:20` states it as a known design property — "No hard delete exists for a Referral (GDPR design, mirrors Person) — cleanup terminalises the tip via Decline" — and `:97-102` does exactly that. So the original wording, "has no cleanup at all — not inline, not in the backstop", read as a spec omission; it is a property of the domain. **What bounds the damage:** the disposable run database (ADR-045); no `@@unique` on `Referral` (only `@@index` on `userId`, `(userId, status)`, `tipsterId`, `targetCompanyId`); no per-user cap at all (`referral.actions.ts:505` documents the list as deliberately un-paginated because counts are network-bounded); measured `Referral +1` per run. `invisible` under `IntraRunVisibility`. **Severity MEDIUM → LOW**, for the same reason as E2E-B22. **Same open question:** corrected `KNOWN_DEBT` reason (`scripts/check-e2e-residue.sh:94`) vs `docs/NOT-PLANNED.md`. | Open |
| E2E-B24 | MEDIUM | **Narrowed to one model, and the rest is now ENFORCED rather than excused.** As filed this covered `Task` (+6 per run), `Activity` (+1), `ActivityType` (+1), `Tag`, and the resume children `Summary`, `WorkExperience`, `Education`, `LicenseOrCertification`, `OtherSection`. `Task` and `Activity` were fixed 2026-09-04 (`278ad940`, `8c72d743`) — neither was the thrown-assertion shape described here, both were E2E-B40's aria-hidden shape. **What this row asked for and had not been done: deleting the KNOWN_DEBT entries.** Its own text says it — while an entry sits in the array the gate PRINTS the model and carries on, so a regression in an already-fixed model is classified as known debt instead of failing the run. Done 2026-09-08: nine entries removed (`Tag`, `Task`, `Activity`, `ResumeSection`, `ContactInfo`, `Summary`, `WorkExperience`, `Education`, `JobStatus`), each measured at zero across four consecutive full runs (two production, two dev) and each having a cleanup path that runs on the FAILING path too — an `afterEach` net, not an inline delete. A full production run after the removal is 112/112 with `[residue] OK`, so the gate now enforces those nine. **What remains is `ActivityType` alone, and it is structural:** `E2E Activity Type` leaks +1 every run, there is no delete path in the application, and no admin tab exists for it — so the sweep that fixed `Activity` cannot reach it. It can only be reduced by creating fewer distinct rows, which both specs now do. `Job` was deliberately KEPT in the array for a measured reason: it leaked +1 in the one of four runs that had a test failure and zero in the other three, which is exactly the inline-delete-skipped-on-throw shape, so enforcing it would add a second red line to every genuine failure. | **Open**, narrowed to `ActivityType`. |
| E2E-B25 | MEDIUM | **FIXED — and its subject was measured clean twice before this row was closed.** As filed: `keyboard-ux.spec.ts` created reference data with no `E2E ` prefix and no cleanup (`KBTest Title/Co/Loc`, `KBRapid`, `KBMobile`, plus tags), and `profile-crud.spec.ts` did the same with `Software Developer`, `company test`, `location test`, `Boston`, `Cambridge`. **profile-crud's half was fixed 2026-09-04** (`8c72d743`): its teardown was blind to the rows it was deleting (E2E-B40), and with that repaired the spec ends at zero for `Resume`, `ResumeSection`, `ContactInfo`, `Summary` and `Location`. **keyboard-ux's half is fixed too** — it now sweeps its reference rows through `sweepReferenceGroups` over all four admin tabs, including `Tag`, and `Tag` has consequently been removed from `KNOWN_DEBT`. **The measurement that closes it, and the reason this row was dangerous.** The residue gate keys on a MODEL, so it printed `Resume`/`JobTitle`/`Company`/`Location` against this finding — whose subject is keyboard-ux — for weeks. Reading the row NAMES out of the kept run database on 2026-09-08 (112/112 production run) gives `E2E Resume <uid>` ×5, `E2E Job <uid>`, `E2E Company <uid>`, `E2E Location <uid>`, `Software Developer`, `Senior Engineer` — and **not one** `KBTest`, `KBRapid` or `KBMobile`. keyboard-ux leaves zero rows. That is the second independent measurement saying so; the first, on 2026-09-05, was written into `scripts/check-e2e-residue.sh` as a warning and never reached this row. The four models are re-pointed to `E2E-B38`, which owns them. | **Fixed** — both halves; subject measured at zero rows 2026-09-05 and 2026-09-08. |
| E2E-B26 | LOW | **CLOSED 2026-09-04 — the premise was destroyed, not fixed.** As recorded: `"E2E Corp"` was **permanently undeletable** because purge step 6a required `workExperiences: { none: {} }` (`cleanup-stale-data.ts:235-243`) while `WorkExperience.resumeSectionId` is optional (`schema.prisma:213`), so deleting the resume only NULLed the link and left a work experience pointing at the company — visible to the filter and permanently guarded out by it. **Three things closed it.** (1) `a0f9a320` deleted `e2e/cleanup-stale-data.ts`; there is no between-runs purge and therefore no step 6a. (2) Cross-run accumulation is impossible under ADR-045 — each run gets a fresh copy of the template and the run database is replaced at the next provision. (3) The row is now owned: `profile-crud.spec.ts:691` registers `companyName` in `createdCompanies` (added `0c31fed2`) and the `afterEach` at `:241-337` sweeps it, deleting resumes FIRST and saying why at `:291-294`. **What survives is a different, benign thing** — the application has its own version of the guard. `deleteCompanyById` refuses while a `WorkExperience` references the company (`company.actions.ts:351-361`), but that count is scoped through `ResumeSection → Resume → profile → userId`, so once the resume is gone the orphaned work experience no longer matches and the delete succeeds. That is the opposite outcome to the purge's unscoped `none: {}`, and the ordering in the teardown exists for exactly it. **Unverified:** no measurement shows `Company` reaching zero on a profile-crud run — E2E-B25's measured list (`Resume`/`ResumeSection`/`ContactInfo`/`Summary`/`Location`) omits it, and `Company:E2E-B25` remains in `KNOWN_DEBT` for keyboard-ux's `KBTest Co`. Needs a quiet full run to confirm the sweep lands. | Closed |
| E2E-B27 | MEDIUM | **FIXED — and the invariant it produced was holding by spelling, which is now also fixed.** As filed: `keyboard-ux.spec.ts:714-727` wrapped an entire selection block in `try { … } catch { console.log("Note: …") }`, so when the option never appeared every functional assertion was skipped and the console-error oracle was the only surviving assertion — green, and measuring the absence of console errors on a page nobody had interacted with. Two siblings at `:766` and `:866`. **All three sites are repaired**, verified against the current file 2026-09-08, and each repair draws the line in the right place rather than deleting the try: at `:1164-1171` and `:1210-1215` only the AVAILABILITY PROBE may `test.skip` (the combobox fetches `/api/eures/locations`, a proxy to a service this suite does not control), while the click and its assertion must be able to fail; at `:1323-1329` there is deliberately no try at all, because job sources are seeded (`prisma/seed.ts`) and a missing option list is a defect, not an outage. A `test.skip` reports NOT RUN; the `console.log` it replaced reported PASSED. **The invariant became a checked one** in `__tests__/e2e-no-swallowed-assertions.spec.ts` (`ConsoleOracleIsNeverTheOnlyAssertion`, `specs/e2e-test-infrastructure.allium`), with the opt-out as a `// swallow-ok: <reason>` marker at the site rather than an allowlist that would drift. **What was still open, found while verifying this row:** that check tested `tryBody.includes("expect(")` — a rule about SPELLING, not behaviour. `try { await deleteJob(page, t) } catch {}` swallows every assertion inside `deleteJob` and contains no `expect(` of its own. Measured: **8 such catches in `e2e/`**, all of them cleanup nets that would have been marked had anyone been asked, none a test body — so the invariant held by the accident of how the code was written. The check now also resolves helpers declared anywhere in `e2e/` whose body asserts, the 8 sites carry a stated reason, and a third self-test case arms the new shape (a detector never seen to fire is not known to work). Every added line in the specs is a comment; no test behaviour changed. | **Fixed** — sites repaired earlier, check tightened and sites annotated 2026-09-08. |
| E2E-B28 | MEDIUM | **FIXED — both halves, and the row had gone stale describing code that no longer exists.** As filed: the console oracle's window was wider than the behaviour it judged (collector at the top of the test body, assertion at the end, so 2-3 full page loads performed as SETUP were inside the window — which is how a dev-server hydration warning came to fail an interaction test, E2E-B11), and its three filters — `favicon`, `404`, `Failed to fetch` — had no recorded rationale anywhere, with a bare `404` substring able to suppress genuine app failures whose message embeds the status. **What replaced it.** `98f4ce4a` gave the oracle an explicit window: `mark()` opens it immediately before the act phase and `sinceMark()` reports only what arrived after, with `sinceMark()` WITHOUT a preceding `mark()` reporting everything — so forgetting the mark makes a test noisier, never silently blind (`keyboard-ux.spec.ts:420-434`, which names this finding). Verified 2026-09-06: 7 collectors, 7 `mark()` calls, 7 `sinceMark()` assertions — every site is inside the act phase. The filters are now anchored regexes with stated reasons rather than substrings: `BROWSER_TRANSPORT_ERROR` (`/^Failed to load resource: net::ERR_/`), `FETCH_TRANSPORT_ERROR` and `BROWSER_RESOURCE_404` (`/^Failed to load resource: the server responded with a status of 404\b/`, i.e. Chromium's own resource message). The substring concern is therefore answered at the root: an application error that merely CONTAINS 404 — `api/logos/[id]/route.ts:67` returns `{error: "Not found"}` with status 404 — cannot match a pattern anchored to Chromium's wording. Transport errors are no longer discarded either; they are reported separately as the evidence that the server went away mid-test (E2E-B35). **What is still unmeasured, and says so in the code:** `e.includes("favicon")` is the one surviving substring, kept from the original filter, with a comment recording that Playwright's `msg.text()` for a resource-load failure carries no URL, so it may well match nothing. It is retained rather than deleted because only a run could prove it dead and the 404 rule already covers the case it named. **Observed since:** across 15 production runs on 2026-09-06 that executed this file's tests, no `sinceMark()` assertion fired. Consistent with the fix; not a proof, for the same reason spelled out in E2E-B11. | **Fixed** by `98f4ce4a`; verified against the current file 2026-09-06. |
| E2E-B30 | MEDIUM | **FIXED (`ea0efaec`) — the suite had an undeclared dependency on data nobody seeds.** `prisma/seed-e2e.ts` now creates three staged vacancies as part of the run template, so the precondition is declared rather than inherited from the developer's database. `e2e/crud/staging-details-sheet.spec.ts` hard-fails (deliberately, per its M-T-03 header) when the seed user has no `StagedVacancy`, and tells the reader to run `bun run seed-dev`. That script does not exist — `package.json:122-124` defines only `prisma.seed` → `bun run prisma/seed.ts`, and `prisma/seed.ts` creates a user, job sources, statuses, a profile and a resume, but **no staged vacancy**. The spec passes today only because the developer's working `prisma/dev.db` happens to hold 56 such rows from ordinary use. On any fresh database the test fails immediately, which is the correct behaviour of an incorrect setup. Verified 2026-09-04: `prisma/seed-e2e.ts:49-80` declares three `STAGED_VACANCIES` and `:99-122` writes them idempotently for `admin@example.com`; `scripts/e2e-db.sh:80` runs the file as part of the template build and `:43` hashes it into the template's staleness key. | Fixed |
| E2E-B29 | LOW | **FIXED (`4447b9fe`, 2026-09-02).** `uniqueId()` now returns `` `${Date.now().toString(36)}w${worker}` `` (`e2e/helpers/index.ts:20-23`), reading `process.env.TEST_PARALLEL_INDEX` — the env var rather than `test.info().parallelIndex`, because the helper is called from module scope where `test.info()` throws; outside a Playwright worker it is absent and `"0"` is then correct. Original: the identifier was `Date.now().toString(36)` alone — millisecond resolution with **no worker discriminator**, while `playwright.config.ts:19,22` runs `fullyParallel: true, workers: 3` locally. Three workers entering the helper in the same millisecond produced identical names, and the suite's only protection against collision IS the name. Never observed; it was structurally possible, which is why it was fixed rather than waited for. | Fixed |

| E2E-B31 | MEDIUM | **FIXED (`8862c00c`) — three tests waited for a table that cannot exist.** `job-detail-panels.spec.ts`'s `navigateToJobsTable` waited for `locator("table")` BEFORE the test created its job. With no jobs, `JobsContainer.tsx:447-449` renders `KanbanEmptyState` and there is no `<table>` at all. Invisible while the suite shared the developer's database (which held four leftover jobs); all three tests failed at once, 10 s each, on the first run against a disposable one. Fixed by splitting the helper: `navigateToJobsTable` does what its name says, `navigateToPopulatedJobsTable` adds the wait for the three callers that expect a row. Verified 11 passed / EXIT=0. | Fixed |
| E2E-B32 | MEDIUM | **FIXED — `selectOrCreateComboboxOption` cost ~11 s per value it had to CREATE.** The helper waited up to 5 s for an option list that stays empty when "Create:" is the only entry (its own comment at `e2e/helpers/index.ts:210` said so), then let `exact` and `partial` expire at 3 s each, before clicking the create entry. Free while the database was full of leftovers and almost every value already existed; with a per-run database, creating is the common case. Measured on `profile-crud.spec.ts:327`, which creates four values: **14.2 s (leftover data) → 60.2 s timeout (fresh) → 25.0 s** after racing the create entry against the option list and short-circuiting when no option exists. The old chain remains as the fallback for every ambiguous case. | Fixed |

| E2E-B33 | MEDIUM | **FIXED — the second consecutive E2E run in one worktree silently used the previous run's server.** Found by the spec agent reading `0e1b5fca`, then reproduced. `dev-e2e.sh` acquires the port lock BEFORE it stops anything (right for a direct call: it refuses rather than killing someone's server), but `test-e2e.sh` always replaces — so run two met its own previous server holding the lock, `dev-e2e.sh` exited 75 into a backgrounded `nohup` whose status nobody read, and the readiness poll then answered YES **against the old server**, while `e2e_db_provision_run` had already pointed the Playwright process at a fresh database that server had never opened. Two databases in one run, no message saying so. Fixed by stopping the incumbent in the wrapper, confirming the lock is actually free (`devserver_wait_lock_free`), and checking the starter's exit status. | Fixed |
| E2E-B34 | LOW | **FIXED — `devserver_stop` waited for the wrong event.** It waited until the PORT was free, but the port frees when the LISTENER dies while the lock is held by the tree ROOT, which outlives it by a beat. The stop returned with the lock still held, so the next start was refused and the wrapper then waited its full 150 s for a server that was never going to start. Now waits for the processes themselves via `kill -0`. | Fixed |
| SPEC-B1 | LOW | **`allium check specs/` reports 3 errors that no commit caused.** `allium` was upgraded from 3.2.3 to 3.6.1 on 2026-09-02 (~18:45); the new binary enforces `rule.invalidTrigger`, which flags `specs/cv-document.allium:574,594,625` — `mode` trigger parameters written in `name: value` form. Measured 0 errors under 3.2.3 the same afternoon, so the change is the toolchain, not the specs. Needs an `allium:tend` pass on that file. Recorded because the obvious reading — "someone broke the specs" — sends the next reader looking for a commit that does not exist. | **Fixed** by `8eba83a8`, which ran the `allium:tend` pass this row asked for (`specs/cv-document.allium:589,610,642`); `allium check specs/` reports 0 errors. The row stayed Open through the `d48b589b` recount two commits later — the arithmetic was right, this status was not. |
| SPEC-B3 | LOW | **`allium check` and `allium analyse` are both blind to reference integrity — but the language server is not.** `specs/module-lifecycle.allium` declares surface `NotificationInbox` with `provides: MarkAsRead / MarkAllAsRead / DismissNotification`, and no rule anywhere listens for those triggers (`grep -c "when: MarkAsRead"` = 0). `allium check specs/` reports **0 errors** and `allium analyse` names none of them; the Allium LSP reports all three as `allium.surface.undefinedProvidesTrigger`. Found 2026-09-05 while tending the spec — the diagnostics fired on a diff that added only comment lines, which is how the pre-existing defect surfaced. | **Fixed** by `3663e8cc`, and the resolution was an overpromise rather than three missing rules. `surface NotificationBell` in `specs/notification-dispatch.allium` already advertised the same three operations with the identical dangling-trigger defect, unreported — so writing rules in module-lifecycle would have left one user action advertised by two specs at two arities. `NotificationInbox` was removed (reading is outside that spec's scope) and the three rules added beside the surface that needs them. A third defect surfaced while making them reachable: both `NotificationBell` and `NotificationPreferencesPanel` bound `where user_id = viewer.email` where the owner is matched by id — String on both sides, so it type-checked and matched nothing. The tooling lesson STANDS and is not closed by this row: `allium check` and `allium analyse` saw none of the three, the language server saw SOME of them — and the sentence that stood here, that it saw all of them, was wrong and is the third tooling claim this session had to withdraw. Corrected 2026-09-05 against measurement: the LSP has ONE rule that holds up (`undefinedProvidesTrigger`, corroborated directly), one mixed (`undefinedPath`), and a rule-shape family that is mostly noise — THIRTEEN false positives were confirmed against both the file text and `allium parse`, including `CacheHit must define a when: trigger` where the `when:` is present, and an unrecognised import alias that is declared at `gdpr-data-rights.allium:19`. It also fires only on files being edited, so it can never sweep 38 specs. It is not a reference-integrity oracle. What DID find the eleven further defects was a purpose-built linter over `allium parse` output, preserved untracked at `tools/allium-refcheck/`; the invariant-collection class (`FallbackChains`, `ExportMetadatas`, `Categories`) is invisible to the LSP and to `allium check` alike. The gap is real; the tool that closes it is a script, not the language server. |

| E2E-B35 | LOW | **`keyboard-ux.spec.ts` is a timing-sensitive cluster, not a set of independent flakes.** Across four full runs its tag/combobox tests failed in varying combinations (`:260`, `:315`, `:353`, `:454`, `:679` — line numbers as of filing; the file has grown to 1,371 lines since and they no longer resolve) and every one of them passed in isolation, repeatedly, in a fraction of the time (2.5 s isolated vs 13.8 s to timeout in a full run). The file is the largest in the suite (22 tests) and asserts on `sr-only` announcements with 5 s polling budgets — the first thing a loaded machine breaks. Recorded as one finding so the next run does not re-diagnose each member separately. The real lever is not the timeouts: the dev server reaches 4-5 GB RSS during a run, and full-run duration moved 15.1 → 19.2 min between two runs with identical code. | **Fixed** — closed 2026-09-07 with 7 valid full dev runs (0 keyboard-ux failures in 154 executions); the one member ever proven real was `E2E-B43` and is fixed. History kept below, because its corrections matter more than the original text. *Status as it stood on 2026-09-07 morning:* **Open — but the lever this row asked for now exists, and one of its two central claims does not survive.** (1) *The lever.* "The dev server reaches 4-5 GB RSS" was a hypothesis in this entry; it is now measured and named — React's development Flight build retains ~2,749 objects per request with no runtime opt-out (`E2E-B42`) — and it is REMOVABLE rather than merely understood: `E2E_PROD=1 ./scripts/test-e2e.sh` loads no development Flight bundle. Measured 2026-09-06, same file, same host: 4 dev runs at 2.3-2.4 min against 3 production runs at 1.2-1.3 min for the same 30 results, **zero `timedOut` results in any production run**, and a full production suite of 112 passed in 12.6 min against dev's 111 passed / 1 failed in 20.2 min. (2) *The claim that does not survive.* "Every one of them passed in isolation" is not a safe general rule, and treating it as one costs a diagnosis. `E2E-B43` — a member of exactly this cluster — failed the file run in ISOLATION 6 times out of 6 under a production build, and passed only when reduced to a single test with `-g`, or when `--trace=on` was added. Both of those change timing. Isolation is therefore a weak oracle for a race: it can hide the defect it is being used to rule out, and here it hid a genuine product defect for as long as the test existed. The honest reading of this row is now "a cluster whose members are timing-sensitive AND may individually be real", which is a different instruction to the next reader than "flakes". **Re-measured 2026-09-07, at the right scope.** Two FULL dev runs — because this row's four failures came from full runs, and the isolated file runs done earlier that evening say nothing about it, which is the whole point of correction (2). Result: **zero keyboard-ux failures in 2 of 2**, 0 `timedOut` results, 111 passed / 1 failed in 27.0 min and 112 passed in 23.2 min, against production's 112 passed in 12.6 min. Two clean runs do NOT close this — at the ~1-in-3 rate this row records, two clean runs happen 44% of the time — but they move it from "known active" to "not reproduced in 2 runs", and the one failure that did occur was not a member of the cluster. **Each dev run still spent one watchdog restart**; a production run has none by construction, so the mechanism this row blamed is still live on the default path. The lone dev failure was `job-detail-panels.spec.ts:440` (Status History list not visible) in the run that also restarted — the same signature this repo has already misdiagnosed twice — and it could NOT be correlated, because `/tmp/jobsync-e2e-dev.log` is truncated by the next run while the wrapper's own banner tells the operator to grep exactly that file. Fixed in passing: `scripts/test-e2e.sh` now keeps each run's server log beside its JSON report. That failure is unattributed and stays that way here rather than being written up as a watchdog casualty on the strength of a coincidence. **Closed 2026-09-07 evening, measured at the right scope.** Seven valid FULL dev runs since the B43 fix (`ff06f705`): the morning pair above plus five more the same evening (runs 2-6 of a six-run loop; run 1 is excluded and recorded below). keyboard-ux: **0 failures in 7 × 22 = 154 executions**, 0 `timedOut` results in any of the seven, wall clock 19.1-19.9 min for the five evening runs. Under the ~1-in-3 per-run rate this row recorded, seven clean runs happen 5.9% of the time — enough to say the cluster AS FILED is no longer active, and not enough to exclude a rarer member: seven clean runs are consistent with anything up to roughly a 1-in-3 rate at the 95% bound (rule of three), so what is excluded is the recorded rate, not the existence of a latent race. What the evidence does NOT establish: that the other 21 members are free of a B43-shaped defect (only that none showed one in 154 executions), and anything about the production path beyond the 112/112 already recorded, since all seven were dev runs. What would reopen it: any keyboard-ux failure in a full dev run — and it would arrive attributable, because every run was sampled at 500 ms (now `E2E_DB_WATCH=1`, `scripts/e2e-db-watch.sh`) and the wrapper now places each watchdog restart against the test window it fell into (`scripts/e2e-attribute-restarts.sh`). Disposition: the one member ever proven real was a product defect and is fixed under `E2E-B43`; the cluster claim is measured at 0 of 7; the mechanism this row blamed is `E2E-B42`, real, and lands elsewhere — next two paragraphs. **The run that was excluded, and why.** Run 1 of the loop took 608.7 min (09:47-19:56): 41 `timedOut` of 112 results, 45 failed, the wrapper's own CONTENDED banner. Its server log shows the event loop blocked from 10:22 to 11:38 (node-cron reported the 10:30 and 10:45 executions missed, at 11:38:39) and the host loadavg stood at 46 while the last tests ran at normal speed — contention this container's guard cannot see (CLAUDE.md § resource discipline). Every keyboard-ux test timed out in it. The driver refused it as a reading; it is listed so nobody re-derives it. **`job-detail-panels.spec.ts:440`, attributed — the failure the morning pair left unattributed.** It failed in 4 of the 5 evening runs (2, 3, 5, 6) and passed in run 4, and in ALL FIVE the single watchdog restart fell INSIDE that test's window: 20:06:59, 20:26:45, 20:47:28, 21:07:24, 21:27:52, at 1,620 / 1,631 / 1,649 / 1,638 / 1,619 logged requests of ~3,110-3,150 per run. The restart is request-count driven (`start-server.js:233` compares used heap with 80% of the cap after every request) and the single-worker suite order is fixed, so it lands on the same test each run; whether that test fails depends on what was in flight at that second. Run 2: `page.waitForURL` → `net::ERR_CONNECTION_REFUSED` at 20:07:01 — the port was closed between the watchdog line at 20:06:59 and `Ready` at 20:07:06 — with the status change in the run database at 20:06:58.2 and its audit line at 20:06:58.1, so the server had answered everything before it. Runs 3, 5, 6: the restart hit the test's cleanup instead — `deleteJob` → `navigateToPopulatedJobsTable`, `locator('table')` not visible within 10 s (spec line 133), the jobs-table GET logged one second before the watchdog line. Run 4: the job was already deleted (rows gone at 20:47:27.7) and the afterEach's `/dashboard/profile` load is what the restart hit; nothing was asserting, so it passed. Five for five, three symptoms, one mechanism, and the sampler ruling out "the server refused it" each time: not a coincidence. Remedy unchanged — `E2E_PROD=1` has no watchdog. Recorded under `E2E-B42` as that row's measured landing zone. |

| SPEC-B2 | MEDIUM | **The spec and the code disagree about the run database's lifetime, and the same author wrote both today.** `specs/e2e-test-infrastructure.allium:552-568` `DiscardRunDatabase` requires the file to be REMOVED when the run ends, "on every terminal outcome, including a failed or interrupted run", because "a suite that leaves its database behind on failure has reintroduced exactly the residue this design removes". The code keeps it (`E2E_KEEP_RUN_DB=1` default, `scripts/e2e-db.sh`), and ADR-045 states that as a deliberate decision with reasons (a red run stays inspectable; no live server left holding an unlinked inode). The brief that produced the rule said "removed after the run"; the implementation went the other way and nobody noticed. Found by an independent reviewer reading both, not by either author. Resolve in one direction: either the rule becomes "discarded at the next provision" or the code discards at exit. | **Fixed** by `ba43c05d` (2026-09-02), which rewrote `rule DiscardRunDatabase` to fire on `TestRunStarted` and added the sibling `rule DiscardRunDatabaseAtExitIsOptIn` (`specs/e2e-test-infrastructure.allium:726`, `:786`). The code agrees: `scripts/test-e2e.sh` keeps the run database by default and discards at the next provisioning. `8eba83a8` later claimed this fix in its subject line while its diff shows 134 insertions and ZERO deletions, so it cannot have rewritten anything — the row then survived the `d48b589b` recount. Same mis-tracking as SPEC-B1. |
| E2E-B36 | LOW | **Eight comments still explain test behaviour by reference to a file deleted today.** `scripts/test-e2e.sh:151,180,183`, `e2e/crud/automation-wizard-modules.spec.ts:122,159`, `e2e/crud/webhook-settings.spec.ts:236`, `e2e/crud/settings-api-keys.spec.ts:175,220` cite `e2e/cleanup-stale-data.ts` — its step 0a, step 0b, step 13 — as the reason a precondition holds or a leak is tolerable. The file is gone; the reasons are now false. `automation-wizard-modules.spec.ts:129` even puts one in an assertion message a future reader will act on. Claims about code, unmaintained by anything — the failure mode this session named. (The ADR-043/045 references are historical and correct.) | Open |

| E2E-B37 | — | **NOT A DEFECT — a falsified assumption, recorded so it stays falsified.** Reclassified 2026-09-04: nothing here is fixable, because nothing is broken. What this entry records is that an inference drawn from a measurement was wrong, and the correction is already load-bearing in `specs/e2e-test-infrastructure.allium`, where `FixtureOwnedTeardown`'s `@guidance` names E2E-B37 as the reason its first narrowing was withdrawn. It is counted as neither open work nor a fix. The lesson, unchanged: **"ends at zero" was a property of the run being green, not of the spec owning its rows — measured, not argued.** The Phase 2 measurement showed `SmtpConfig`, `CompanyBlacklist` and `Question` at zero residue and that was read as evidence those specs clean up reliably. A reviewer pointed out that all three clean up **inline at the end of the test body**, the path a failed assertion skips, and that only the webhook and API-key specs have `afterEach` nets. Falsification test run 2026-09-02 21:40: one `expect(true).toBe(false)` injected between save and the inline delete in `smtp-settings.spec.ts:202`, run through the wrapper — `SmtpConfig` goes from 0 to **1**, an active per-user singleton left in the run database. The suite absorbs this particular case because `ensureEmptyState` (`:72-78`) deletes an existing config before each SMTP test; a model without such a guard would not be absorbed. The inference "the capacity-bound models already own their rows, so a fixture rewrite buys nothing" therefore does not follow from the measurement. What survives: those rows still die with the run database, and a count-based residue gate DOES catch this shape (an INSERT). It does not catch the UPDATE shape — see the `deactivateModule` upsert in the same session's notes. | Corrected assumption |

| E2E-B38 | MEDIUM | **The row this finding has owed since 2026-09-03, written from measurement 2026-09-08.** Until now it existed only as four `KNOWN_DEBT` comments in `scripts/check-e2e-residue.sh` and was counted open on the strength of them. Subject: specs that delete their rows INLINE at the end of the test body, the path a thrown assertion skips. Measured owners, read from the kept run database of a 112/112 production run rather than inferred from the model: `Automation +5` (`E2E Automation <uid>`, `E2E Edited <uid>`) and `Resume +5` carrying **the same five uids** — automation-crud, whose own `afterEach` warns `leaked automation survived cleanup … it stays in the run database and keeps its resume undeletable`; `JobTitle +3`, `Company +1`, `Location +1` (job-crud's `E2E Job/Company/Location <uid>` plus profile-crud's unprefixed `Software Developer` and `Senior Engineer`); `JobSource +1`, the row `Manual`, the tenth against nine seeded in `prisma/seed.ts`; `Job +1`, but only in the one run of four that had a test failure. **What is NOT the cause, ruled out rather than assumed:** `deleteAutomation` (`src/actions/automation.actions.ts:355`) refuses while a run is in progress, and the run database contains **zero `AutomationRun` rows**, so the application did not refuse. The failure is inside `deleteAutomation` in `e2e/crud/automation-crud.spec.ts:129-159`, which catches everything; WHICH of its four steps fails — the dropdown, the Trash2 menuitem filter, the alertdialog's `.last()` button, or the `getByText(name)` disappearance assertion — is unmeasured, and no guess about it belongs in this row. Next step: instrument that helper to report the step it lost, one spec run. | **Open.** |
| E2E-B39 | MEDIUM | **FIXED (`3fe7412a`) — and it was never a test defect.** This row spent days calling it a test-isolation problem, and the tests were right to fail. `Combobox` made a just-created option visible by calling `options.unshift(result)` — mutating the array its PARENT owns (`ComboBox.tsx:63`) — while deriving the trigger's text from that same array (`:113`). A parent that legitimately replaces it (`AddExperience` refetches on mount) dropped the created row while `field.value` still held its id, so `find` missed and the trigger rendered `""`. The row was in the database throughout, which is why every investigation that checked the DATA found nothing. Fixed in the component — created options are held in local state, prop consulted first so a later rename still wins — which repairs all five call sites rather than the one the suite exercised. The ordering sensitivity below was the AMPLIFIER, not the cause: a loaded server widens the window between the create resolving and the fetch landing. Making the spec independent, which this row previously proposed and I nearly did, would have hidden it. Diagnosis: `docs/e2e-b39-analysis.md`. Regression test at `__tests__/ComboBox.spec.tsx`, verified red against the unfixed component. Historical record follows. **Widened 2026-09-06: it is TWO tests, and the second proves the diagnosis.** `profile-crud.spec.ts:448` "edit experience dialog opens and cancels" fails the same way — `getByLabel('Company')` asserted to contain `"company test"`, trigger reads `""`. It was GREEN at 00:51 and RED at 01:31 with exactly one commit between the runs (`31ed962b`), which touches only `e2e/crud/kanban.spec.ts` and cannot reach this file except by changing what ran before it. That is the order dependency this row names, demonstrated rather than inferred. `profile-crud.spec.ts` "add work experience" selects `"Software Developer"` in the Job Title combobox and asserts `toContainText`; the trigger reads `""`. Originally recorded as a cross-test dependency inside the file. Re-measured 2026-09-04 with a sharper instrument: the spec run ALONE is 16/16 green, and the same spec run after `task-crud` + `activity-crud` fails at that assertion — **twice, and identically on the unmodified baseline**, so it is ordering-dependent rather than flaky and nothing in the aria-hidden fix causes it. The row IS created either way (`Software Developer` is in the run database after the failing run), so it is not select-versus-create as first described: the create path completes and the trigger does not update. Two defects still meet here — a spec that is not independently runnable (`NoCrossTestDependency`) and a control that stays empty after its own create — and neither is a cleanup hook's to fix. | **Fixed** 2026-09-06 by `3fe7412a`. |
| E2E-B40 | MEDIUM | **FIXED (`278ad940`, `8c72d743`) — and the recorded cause was wrong.** The status filter was blamed: a task that merely left `DEFAULT_STATUS_FILTER` would satisfy `toHaveCount(0)`. Measured against the kept run database it is the modal, not the filter. Radix's AlertDialog calls `hideOthers()` (`@radix-ui/react-dialog/dist/index.mjs:137`), which sets `aria-hidden="true"` on every child of `document.body` outside the dialog portal. `getByRole` consults the accessibility tree, so for that whole window it matches NOTHING and every phrasing of "the row is gone" — `toHaveCount(0)`, `not.toBeVisible()`, `waitFor({ state: "detached" })` — is satisfied by a row still on screen and still in the database. Six of the seven task-crud tests that create a task left it behind after a run Playwright reported as passed; the seventh, the only one that also awaited the delete toast, is the only one whose row is gone. A CLASS, not an instance: four helpers across `task-crud`, `activity-crud`, `profile-crud` and `helpers/resume-fixture` were in that state, including yesterday's fix for the same symptom against `Resume +29`. The rule now lives once, in `rowsByText` (`e2e/helpers/index.ts`), paired with `expectToast` because the DOM only refreshes after the container's reload — the toast is the only signal that comes from the round trip. Measured, same protocol both sides: `Task +6 -> 0`; and profile-crud alone `Resume +3 -> 0`, `ResumeSection +1 -> 0`, `ContactInfo +1 -> 0`, `Summary +1 -> 0`, `Location +3 -> 0`. | Fixed |
| E2E-B41 | MEDIUM | **FIXED — the guard that refuses to judge a swamped run had never been able to fire.** `1cd54f8d` made the residue gate skip when a run reports 3+ timeouts, reading the count from the Playwright JSON report. Nothing produced one: `scripts/test-e2e.sh:261` invoked `playwright test` with the project's CONFIGURED reporters -- `[["html", {open: "never"}], ["list"]]` (`playwright.config.ts:25`), NOT Playwright's default, and that mis-description is why the fix was written as if the CLI value merely added one more. It replaces (`config.js:90`, `takeFirst`), so `1cd54f8d`'s `--reporter=list,json` silently stopped `playwright-report/` being produced on every default run -- repaired in `64e2f52d`. The reporter then in use was and never set `PLAYWRIGHT_JSON_OUTPUT_NAME`, so the file was always absent — and absent initialised `TIMED_OUT=0`, the one value that lets the gate proceed. The check was therefore strongest exactly when it knew least. Found by using it: the 2026-09-04 full run took 783 minutes wall clock with single tests at 13-14 minutes and **50 of 112 results `timedOut`** (host load average 36), and the gate judged its residue and printed eleven models as debt. Those numbers measure the machine and must not be quoted. Fixed in two halves — the wrapper now produces the report itself (and says so when a caller's own `--reporter` takes that guarantee away), and a missing report is `unknown` rather than zero, which skips. Five branches exercised in isolation. | Fixed |
| E2E-B42 | MEDIUM | **The E2E dev server's memory growth is React's development Flight build, not Turbopack and not application code — MEASURED, after three wrong diagnoses.** `react-server-dom-webpack-server.node.development.js` installs a process-wide `async_hooks.createHook(...).enable()` at module load with no guard, for React's async debug-info / owner-stack tracking. It keeps `pendingOperations: Map<asyncId, node>` where each node is `{tag, owner, stack, start, end, promise, awaited, previous}`. `destroy` removes the MAP entry only; the nodes are ALSO chained to each other through `previous`/`awaited`, and `promiseResolve` builds nodes assigned as `node.previous` that are never entered into the Map at all, so `destroy` cannot reach them by construction. Evidence: a heap-snapshot pair from one server lifetime, 701 MB and 1170 MB heapUsed, 401 requests apart. `object: WeakRef` grew by 1,102,348 (~2,749 per request); every one is referenced by a distinct plain `Object` through a property `promise`, and those Objects chain through `awaited` (1,337,594) and `previous` (950,488). The `stack` and `owner` fields are where the +110 MB of strings and +128 MB of element arrays go. Tools: `tools/next-heap/snapshot-pair.py` (takes the pair), `heap-classes.py --diff` (what grew), `heap-retainers.py` (who holds it); all three carry a `--self-test`. Full write-up: `docs/e2e-dev-server-restart-analysis.md`, second addendum of 2026-09-06. | **Open — upstream, and no code fix belongs in this repo.** Both `app-page.runtime.dev.js` and `app-page-turbo.runtime.dev.js` carry the hook, which is why the earlier webpack measurement grew identically (+672 MB) — the bundler was never the cause. There is no runtime opt-out: no `process.env` test within 200 lines of the `.enable()` in either the stable or the `-experimental` build, because React gates the feature at its own build time. The only remedy that addresses the cause is running the suite against `next build` + `next start` (ROADMAP §8.5 Phase 3), whose blocker is unchanged and separate: `E2E_AUTH_RATE_LIMIT_BYPASS` is deliberately inert under `NODE_ENV=production`. That is a decision to take, not a defect to fix. Mitigation already shipped: `scripts/test-e2e.sh` counts the restarts and says what they mean. **Decision TAKEN 2026-09-06 and the remedy is shipped: `E2E_PROD=1 ./scripts/test-e2e.sh` runs the suite against `next build` + `next start`** (`scripts/prod-e2e.sh`, `scripts/e2e-prod-build.sh`, output in `.next-e2e/`). The retention is structurally absent there, not merely rarer: no development Flight bundle is loaded, and the watchdog itself sits inside `if (isDev)` at `start-server.js:233`. Measured the same day on the same tree — full suite **111 passed / 1 failed in 11.3 min** against dev's 111/1 in 20.2 min, build 1 min 54 s. The auth blocker this row called unchanged turned out to be smaller than the row said: 5 signins per 15 min per IP against three spent, now **two**, because `e2e/global-setup.ts` mints the JWT session cookie instead of signing in. The row stays OPEN because the defect is React's and no code fix belongs here — what this repo now has is a way not to run into it. **Landing zone measured 2026-09-07 (E2E-B35 close-out):** in five valid full dev runs the restart fired at 1,619-1,649 logged requests of ~3,110-3,150, i.e. inside `job-detail-panels.spec.ts:440` every time, which failed in four of the five (`ERR_CONNECTION_REFUSED` once, a hung jobs-table load three times) and passed once when nothing was in flight. `scripts/test-e2e.sh` now prints which test each restart fell inside (`scripts/e2e-attribute-restarts.sh`), so the next casualty needs no correlation by hand. |

| E2E-B43 | MEDIUM | **FIXED — a real product defect, found only because the suite ran against a production build.** `TagInput` cleared its search field from INSIDE the `startTransition` that wraps the `createTag` round trip, i.e. after the server answered. A transition write is low priority, so it could commit after the user had already typed the next skill — and because the field is a CONTROLLED input, React wrote that stale `""` back into the DOM as well. The typed text disappeared, and the next Enter had nothing to act on: `TagInput.tsx:137`'s guard was false, so there was no request, no chip, no error and no toast. The entry was simply lost. A user typing quickly loses a skill; the suite loses it every time. **Fixed** by clearing the field synchronously when Enter is accepted (and putting the text back if the create fails), plus reading `e.currentTarget.value` in the handler rather than state. Three regression tests in `__tests__/TagInput.spec.tsx`, all verified RED against the unfixed component. `ComboBox.tsx` was checked for the same shape and does NOT have it: its clear at `:115` is synchronous with the keystroke and its popover closes after one create. **Measured, not argued.** `keyboard-ux.spec.ts:755` failed **6 production runs out of 6** and passed **3 dev runs out of 3**; the failing iteration moved between the second and third skill. After the fix: three production runs of the whole file, 30 passed each, and the full suite green. A 500 ms sampler over the run database recorded `KBMulti1` and never `KBMulti2`/`KBMulti3` across the whole 26 s failure window, which proved the create never reached the server and killed the competing hypothesis of a failed `createTag` (`handleError` logs, and the production server log is clean). Two runs that did NOT reproduce are explained by the same mechanism rather than contradicting it: `-g` on the single test and `--trace=on` both change timing. **Three claims withdrawn on the way, recorded so the next reader does not re-walk them.** (1) "The shape of a `TagInput` remount — no chips, popover closed" was read out of `error-context.md`, which is NOT a failure snapshot: `_takePageSnapshot` runs only from `willCloseBrowserContext` and `didFinishTest` (`node_modules/playwright/lib/index.js:575,615`), i.e. after this file's `afterEach` navigated to the admin page. The "Skills Tags" heading in that snapshot is `admin.skillsTags`. Now documented in `e2e/CONVENTIONS.md` § Reading a failure's artefacts. (2) An experiment that queried the run database AFTER the run was inconclusive by construction — `deleteAdminReferenceRow:135` treats a row that was never written as already gone, so both hypotheses produce an empty table and no warning. (3) The first fix read `e.currentTarget.value` and reproduced 1 for 1, because a controlled input's DOM value follows the last commit; that failure is what identified the real mechanism. | **Fixed** |

Also noted, not bugs: `login()` in `e2e/helpers/index.ts` is a dead export (no spec imports it;
the two smoke specs and `global-setup.ts` each inline their own), and `ensureEnglishLocale` is
duplicated in 16 spec files.

### Section recount, 2026-09-04

Counted from the rows, not from the heading. This section now holds **22 findings**: `E2E-B22`
through `E2E-B41` (20 ids) plus `SPEC-B1` and `SPEC-B2`. The heading's "18 found" was correct for
the audit itself (`E2E-B22..B37` + the two `SPEC-B`); `E2E-B38..B41` were added on 2026-09-03/04 and
are counted here.

| Disposition | Count | Ids |
|---|---|---|
| Fixed | 8 | B29, B30, B31, B32, B33, B34, B40, B41 |
| Closed (premise destroyed) | 1 | B26 |
| Corrected assumption (not work) | 1 | B37 |
| Open | 12 | B22, B23, B24, B25, B27, B28, B35, B36, B38, B39, SPEC-B1, SPEC-B2 |

8 + 1 + 1 + 12 = 22. *Moved since this recount:* B28, B39, SPEC-B1, SPEC-B2 to fixed (by 2026-09-06) and B35 to fixed (2026-09-07), i.e. 13 fixed / 7 open now; the block table at the top of the file is the current derivation.

**Three of those twelve need adjudication and were left alone deliberately, because the evidence is
one-sided and re-statusing them was outside the pass that wrote this note:**

- **`E2E-B38` has no row in this file.** It exists only as a comment in
  `scripts/check-e2e-residue.sh:103-106,121-125`, where four `KNOWN_DEBT` entries
  (`Automation`, `JobSource`, `JobStatus`, `Job`) cite it and the provenance block explains what it
  records: the debt list was seeded from a document when a full-run measurement of the same thing
  already existed on disk, and the document was the less complete of the two. It is counted as open
  above so the total is honest; it needs a row written from that comment.
- **`E2E-B28`** (console oracle's window wider than the behaviour it judges) — `98f4ce4a` replaced
  `collectConsoleErrors` with a `mark()` / `sinceMark()` handle and moved all seven `keyboard-ux`
  assertion windows to the act phase. The commit says so and names the finding. Not re-measured.
- **`E2E-B36`** (comments citing the deleted purge) — zero references to `cleanup-stale-data`
  survive in `scripts/` or `e2e/` as of 2026-09-04; the last were removed by `31763aa9` (the five in
  `scripts/`) and `98f4ce4a` (six more, one of them assertion text). The single remaining mention,
  `scripts/e2e-db.sh:10`, is deliberate and historical — it describes what the file replaced.

**Recommendation for E2E-B22 / E2E-B23 (Person and Referral), for whoever decides it.**
Recommend **`KNOWN_DEBT` with corrected reasons, not `NOT-PLANNED.md`.**

*For `NOT-PLANNED.md`:* the entries are a genuine, permanent "will not fix" with a clean rationale
(no delete path exists by GDPR design), which is exactly that file's shape — `| # | Vorschlag |
Begründung gegen Umsetzung | Wann es doch sinnvoll wäre |` — and its stated purpose is to stop
reviewers re-surfacing settled questions as new findings. Two rows there would end the matter.

*Against, and why it wins:* `KNOWN_DEBT` is not a backlog, it is the **gate's classifier**. Every
model in that array is a name the residue gate must recognise at runtime; removing `Person` and
`Referral` from it does not mark them not-planned, it makes the next run **fail** on rows that are
correct and expected. So the entries have to stay in the array whatever the disposition, and a
finding that is *cited by a live enforcement mechanism* should be readable where the mechanism
points. `NOT-PLANNED.md` would then hold the judgement while `check-e2e-residue.sh` holds a pointer
to a `docs/BUGS.md` id — two homes, and the drift this session spent its time undoing.

*The concrete change:* correct the two comments in place so they state the cause rather than a
spec's omission — `:93` from "four specs archive but never delete" to something like "no delete
path exists for Person (GDPR design); rows die with the run database", and `:94` likewise. That is
a two-line edit to `scripts/check-e2e-residue.sh`, which this pass does not own.
...
introduced by the OP-B2 fix itself and found by the phase-1/2 review agents. Every claim below was
...
| ID | Severity | Summary | Fix |
|----|----------|---------|-----|
| OP-B1 | HIGH ✅ FIXED 2026-08-23 | **Deleting a Job/Company/Person silently hard-deleted visible, non-terminal CrmTasks.** The orphan prune shipped in `81ad0940` deleted any zero-target note *and task*, justified as "unreachable on every timeline". False for tasks: the board calls `getCrmTasks()` with **no filter** (`CrmTasksPageClient.tsx:101`) and renders the targetless case on purpose (`:214`), and `checkOverdueTasks` (`crm-cron.ts:240`) selects on status+dueDate alone, so an orphaned task stays visible and keeps firing reminders. The prune also deleted `pending` tasks, bypassing `rule DeleteTask` (`crm.allium:987`, terminal-only) — **re-opening the guard W-A1 had just closed**, through a second path. | Task prune removed entirely; a task that loses its last target simply loses its link (`targets.count > 0` is an obligation on the creating action, not a standing predicate). Tests pin that `crmTask.deleteMany` is never called. Commit `d20892f2`. |
| OP-B2 | MEDIUM ✅ FIXED 2026-08-21 | **Deleting a Person, Company or Job orphaned CrmNotes.** `CrmNoteTarget`/`CrmTaskTarget` cascade with their target (`schema.prisma:1186,1225`), so a note whose ONLY target was that entity survived with zero targets — unreachable in the UI (every note read filters by target) while still holding its free-text body. No cleanup existed, and `CreateNote` requires `targets.count > 0`, so the state was unreachable by any legal action. | Two-phase prune in `src/lib/crm/orphan-targets.ts`, wired into all five delete paths. Commits `81ad0940`, `741c904e`. |
| OP-B3 | HIGH ✅ FIXED 2026-08-23 | **Free text about an erased person survived `anonymizePerson` and was still exported.** The prune only reaps records left with NO target, so a note or task that also targeted a Job/Company kept its body. Unreachable in the UI — but the **Art. 15 export emits it**: `collect-user-data.ts:346` reads `crmNote.findMany({ where: { userId } })` with no target filter, selecting `title` and `body`. Ongoing disclosure of data subject to Art. 17 erasure. | Scrub instead of retain, mirroring the existing `crmInterview.notes/outcomeNotes` treatment: notes get a null title + empty body, tasks an empty title + null description, ordered before the target deletes. Tasks are scrubbed and not deleted (see OP-B1). Commit `b08e5645`. |
| OP-B4 | MEDIUM ✅ FIXED 2026-08-23 | **The GDPR erasure wrote no audit entry at all.** `anonymizePerson` (`person.actions.ts:541-688`) called `writeDataAuditLog` nowhere, while a routine `job.delete` is fully attributed with actor id + email; the `contact_deleted` projection deliberately nulls `targetPersonId`, so it cannot serve as the record. Art. 5(2) accountability gap on the one action a controller is most likely to have to demonstrate. | `person.anonymize` added to `AuditAction` + `rule AuditPersonAnonymise` (`audit-trail.allium`), `DataMinimisation` widened to cover it, and the call site added. No sink change needed — `writeDataAuditLog` already drops snapshots outside `SNAPSHOT_ACTIONS`, so "an erasure never carries a snapshot" holds structurally. Commit `b08e5645`. |
...
**Known, deliberately not fixed:** `orphan-targets.ts` lacks `import "server-only"` (sibling modules have it); the prune's count is not reported anywhere (Low, now that the erasure itself is audited). Pre-existing and adjacent, flagged by the security audit but outside this change set: `company.actions.ts:347,357` count across all users and `:365` leaks that count in the error message; `mock.actions.ts:507,510` lack a `userId` filter.
...
| ID | Severity | Summary | Fix |
|----|----------|---------|-----|
| IT-B1 | MEDIUM ✅ FIXED 2026-08-20 | **Anonymized Persons remain selectable in every contact picker.** `getPersons` only filters status when asked (`person.actions.ts:239` — `if (filters?.status)`), and `AddJob.tsx:272`, `InterviewForm.tsx:208`, `PersonDetailClient.tsx:185` all call `getPersons({ pageSize: 200 })` with no status. `anonymizePerson` nulls `firstName`/`lastName` and sets `emails: "[]"`, so an erased contact appears as a **blank but selectable row** — and selecting one re-links an erased record to a new Job / Interview / PersonConnection, re-establishing processing on data that was subject to Art. 17 erasure. Also affects `archived` Persons (clutter, not a rights issue). | **✅ FIXED 2026-08-20.** All four pickers (`AddJob`, `InterviewForm`, `PersonDetailClient`, `TipCaptureSheet`) now pass `status: "active"` to `getPersons`, so an anonymized/archived Person is no longer selectable. Chose picker-side filtering over flipping the `getPersons` default: `ContactsPageClient` defaults `statusFilter="all"` and relies on the fail-open default to render the full management list (incl. an explicit `anonymized` filter), so flipping the shared contract would have broken that view. Regression test tightened in `PersonDetailClient.connections.spec.tsx` (asserts `getPersons` called with `status: "active"`). Commit `5c49bb43`. |
| IT-B2 | LOW | **`CrmActivityLog.details` is rendered to users as a raw JSON blob.** All projections write `details: JSON.stringify({...})` (`crm-activity-logger.ts:128, 191, 332, 346`), and `ActivityTimeline.tsx:143` renders `{String(activity.details)}` with **no `JSON.parse` anywhere in the component tree**. Existing `status_changed`, `application_submitted` and `automation_degraded` entries therefore print e.g. `{"previousStatus":"lead","newStatus":"applied"}` verbatim in the timeline. | **✅ FIXED 2026-08-19 (§F).** New pure helper `src/components/crm/activity-format.ts` `activityDetailText()` parses `details` per `activityType` and renders a localised sentence (referral kind / `previous → new` status via `insideTrack.*` labels; job status values verbatim). Unknown/legacy JSON shapes return `null` → the timeline renders NO details line, so a raw JSON object never reaches the DOM. `ActivityTimeline.tsx` no longer references `String(activity.details)`. 10 unit tests in `__tests__/activity-format.spec.ts`. |
| IT-B3 | LOW ✅ FIXED 2026-08-20 | **`ActivityType.vacancy_promoted` is dead, and the spec disagrees with the code.** `specs/crm.allium` `RecordVacancyPromotion` writes `activity_type: vacancy_promoted`, but the implementation maps `VacancyPromoted` → **`application_submitted`** (`crm-activity-logger.ts:317-334`). Nothing ever writes `vacancy_promoted`, and it has **no `crm.activity.*` i18n key** in any of the 4 locales — so if anything ever did write it, `t(\`crm.activity.${type}\`)` (`ActivityTimeline.tsx:137`) would render an untranslated key, because the label lookup has no fallback (the icon lookup does: `ACTIVITY_ICONS[type] ?? ActivityIcon`). | Not yet fixed. Decide which side is right, then align: either the projection emits `vacancy_promoted` (+ 4 i18n keys) or the spec drops the member. Independently, give the label lookup a fallback so a missing key degrades instead of leaking a key name. | **✅ FIXED 2026-08-20 (spec side).** `specs/crm.allium` `RecordVacancyPromotion` now writes `application_submitted` (promoting a discovered vacancy into a tracked Job IS the application-submitted moment) and the dead `vacancy_promoted` member was dropped from the `ActivityType` enum — the code union never had it. The `vacancy_promoted` NotificationType is a separate, live enum and is untouched. Label-lookup fallback already shipped 2026-08-19 (A-4, `activity-format.ts`). Commit `71da1c52`. |
| IT-B4 | LOW | **The stale-referral sweep attributes a system action to the last human editor.** `flagStaleReferrals` (`crm-cron.ts:115-121`) is a bulk `updateMany` writing only `status: "stale"`, leaving `updatedByType: "user"` / `updatedById: <user>` from whatever the person last did (`referral.actions.ts` sets them on all 4 write paths). So the audit trail claims a user moved the referral to `stale` when a cron did. `ActorType` already has an `automation` member for this (`specs/crm.allium`, ADR-035). | **✅ FIXED 2026-08-19 (§F).** `flagStaleReferrals` (`crm-cron.ts`) rewritten from blind `updateMany` to `findMany`-then-per-row-`update`, setting `updatedByType: "automation"`, `updatedById: null`. Each row also emits `ReferralStatusChanged` (`systemInitiated: true`) carrying its own `previousStatus` + tipster/company links — the same rewrite `specs/inside-track.allium` `ReferralGoesStale` requires. Tests in `__tests__/crm-cron.spec.ts`. |
...
## Session 2026-06-12 — Open BACKLOG bug-list verification + fixes

Verify-against-source pass on the carried "open items". Outcome: **2 of 4 were not real as documented** (index-vs-code drift).
