...
- **Future cleanup (NEW, low priority):** unify `convertResumeForMatch` (runner) with `convertResumeToText` (routes) — the duplication is what let the runner leak slip past the route hardening. Blocked only by a minor type difference (`ResumeWithSections` Prisma shape vs `Resume` profile.model). ~half-day.
