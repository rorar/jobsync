...
//   user             → { email: "user@test.com" } (basic user)
//   webhookEndpoint  → [] (no endpoints → WebhookChannel skips)
//   smtpConfig       → null (no SMTP → EmailChannel skips)
//   vapidConfig      → null (no VAPID → PushChannel skips)
//   webPushSubscription → [] (no subscriptions → PushChannel skips)
const mockUserFindUnique = jest.fn().mockResolvedValue({ email: "user@test.com" });
...
jest.mock("@/lib/db", () => ({
  ...
  default: {
    ...
    // PERF-3: buildDispatchContext queries these tables in parallel
    webhookEndpoint: {
...
describe("NotificationDispatcher", () => {
  ...
  // ---------------------------------------------------------------------------
  // PERF-3: buildDispatchContext mock verification
  //
  // These tests verify that the 6 Prisma tables queried by
  // buildDispatchContext() are properly mocked. Default mock values (null / [])
  // produce a context where email/push/webhook are unavailable, so only the
  // InAppChannel writes. This proves:
  //   1. All tables are reachable (no TypeError on undefined property).
  //   2. The dispatcher completes normally when non-inApp channels report
  //      "nothing configured" via the availability flags.
  // ---------------------------------------------------------------------------
  describe("PERF-3 — buildDispatchContext channel mocks", () => {
    it("dispatches VacancyPromoted without TypeError when all channel mocks return defaults", async () => {
      const event = createEvent(DomainEventType.VacancyPromoted, {
        stagedVacancyId: "sv-1",
        jobId: "job-1",
        userId: "user-1",
      ...
      // InAppChannel must have written despite other channels being unavailable.
      expect(mockCreate).toHaveBeenCalledTimes(1);
    });

    it("buildDispatchContext queries are reachable (not TypeError)", async () => {
      const event = createEvent(DomainEventType.VacancyPromoted, {
        stagedVacancyId: "sv-1",
        jobId: "job-1",
        userId: "user-1",
      ...
      // The mock functions must have been called by buildDispatchContext.
      expect(mockFindUnique).toHaveBeenCalled(); // userSettings
      expect(mockCreate).toHaveBeenCalled();
    });

    it("channels see unavailable config and skip gracefully for ModuleDeactivated", async () => {
      const event = createEvent(DomainEventType.ModuleDeactivated, {
        moduleId: "eures",
        userId: "user-1",
        affectedAutomationIds: ["auto-1"],
