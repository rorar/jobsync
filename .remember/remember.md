...
## State
Session 2026-05-12c: IF-3 + IF-4 + Quick Wins. 7 commits on `main`. tsc clean, 228 test suites, 4458 tests green.
...
## Context
- IF-3 DONE: 3 migrations (`add_crm_cascade_deletes`, `fix_polymorphic_target_cascade`, `cascade_all_polymorphic_targets`). CrmInterview/JobContact → Cascade. ALL 6 polymorphic target FKs (Person/Company/Job × TaskTarget/NoteTarget) → Cascade (join rows). ActivityLog → SetNull, StagedVacancy → SetNull.
